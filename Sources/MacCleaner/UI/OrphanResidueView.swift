...
struct OrphanResidueView: View {
    ...
    private var limitationsNote: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("• 判定方式是「该标识不被任何已安装应用引用」。扫描时会读取每个应用**内部**的全部标识（实测本机 29 个应用共 777 个），只比对顶层标识会大量误判。")
                .font(.caption)
                .foregroundStyle(.secondary)
            Text("• 同时要求 90 天内未被访问 —— 有些应用不在自己的配置里声明所用容器，仅靠标识比对会误报。")
                .font(.caption)
                .foregroundStyle(.secondary)
            Text("• 采用保守策略：宁可漏报也不误删。当前 \(scanner.knownIDCount) 个已知标识。")
                .font(.caption)
                .foregroundStyle(.secondary)
